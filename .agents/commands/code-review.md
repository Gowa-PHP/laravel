---
description: Run code review on a PR or branch with CodeRabbit CLI and GOWA Laravel REVIEW.md Quality Gates
...
2. Rigorously follow the workflow defined in `.agents/REVIEW.md` and the `code-review` skill.
...
5. Execute the package's local automated checks: `composer test` (Pest 3.x) and `composer lint` (PHP-CS-Fixer).
